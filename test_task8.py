...
if __name__ == '__main__':
    ...
    thresholds = [2, 2, 2]
    f_length = [2,3,8]
    w_path = os.path.join( base_dir,f"1_data", f"data", "REMOVEWORDS.csv")
    ...
    for i in range(1, 4):
        ...
        p_path = os.path.join(base_dir,"2_examples","Q8_examples", f"example_{i}", f"people_small_{i}.csv")
        s_path = os.path.join(base_dir,"2_examples","Q8_examples", f"example_{i}", f"sentences_small_{i}.csv")
        e_path = os.path.join(base_dir,"2_examples","Q8_examples", f"example_{i}", f"Q8_example_{i}_w_{window_size[i - 1]}_threshold_{thresholds[i - 1]}_fixed_length_{f_length[i - 1]}.json")
        pc_path = os.path.join(base_dir,"2_examples","Q8_examples", f"example_{i}", f"people_connections_{i}.json")

        # Output JSON files
        output_path_task6 = os.path.join(base_dir,"2_examples","Q8_examples", f"example_{i}", f"task6_output{i}.json")
        output_path_task7_not_processed = os.path.join(base_dir,"2_examples","Q8_examples", f"example_{i}", f"task8_result{i}_not_processed.json")
        output_path_task7_preprocessed = os.path.join(base_dir,"2_examples","Q8_examples", f"example_{i}", f"task8_result{i}_preprocessed.json")
        ...
        print(f"Running Task 8 with raw input for File Set {i}...")
        task7_non_preprocessed = task7.IndirectConnector(
            s_path, p_path, w_path, pc_path, False, window_size[i - 1], thresholds[i - 1],f_length[i - 1],
        )
        ...
        if task7_non_preprocessed_result == expected_result:
            print(f"{Colors.GREEN}Test {i} (Task 8 with raw input): SUCCESS{Colors.RESET}")
        else:
            print(f"{Colors.RED}Test {i} (Task 8 with raw input): FAIL{Colors.RESET}")
        ...
        print(f"Running Task 6 to create preprocessed input for Task 8 for File Set {i}...")
        ...
        print(f"Running Task 8 with preprocessed input for File Set {i}...")
        task7_preprocessed = task7.IndirectConnector(
            output_path_task6, None, None, pc_path, True, window_size[i - 1], thresholds[i - 1],f_length[i - 1]
        )
        ...
        if task7_preprocessed_result == expected_result:
            print(f"{Colors.GREEN}Test {i} (Task 8 with preprocessed input): SUCCESS{Colors.RESET}")
        else:
            print(f"{Colors.RED}Test {i} (Task 8 with preprocessed input): FAIL{Colors.RESET}")
