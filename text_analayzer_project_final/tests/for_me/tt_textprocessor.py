...
if __name__ == '__main__':
    ...
    for i in range(1, 4):
        ...
        p_path = os.path.join(base_dir,f"example_{i}", f"people_small_{i}.csv")
        s_path = os.path.join(base_dir, f"example_{i}", f"sentences_small_{i}.csv")
        e_path = os.path.join(base_dir, f"example_{i}", f"Q1_result{i}.json")

        # Output JSON file for processed result
        output_path = os.path.join(base_dir, f"example_{i}", f"processed_result{i}.json")
        ...
        T = TextPreprocessor(s_path, p_path, w_path, False,1)
